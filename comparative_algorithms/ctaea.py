...
ref_dirs = get_reference_directions("energy", n_dim=NUM_OBJ, n_points=len(ortho_pop))  # 使用正交数据量设置参考点数，作为初始种群和
...
csv_file = open(os.path.join(f'{args.outfile}', f'ctaea_{formula}.csv'), mode='w', newline='')
...
if callback.found_solution:
    ...
    coe = 4
    with open(os.path.join(f'{args.outfile}', f'ctaea-Target_{formula}_components.txt'), mode="a") as file1, \
         open(os.path.join(f'{args.outfile}', f'ctaea-Target_{formula}_properties.txt'), mode="a") as file2:
