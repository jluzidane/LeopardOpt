...
# Generate reference directions: each row represents one reference direction in the objective space.
ref_dirs = get_reference_directions("energy", NUM_OBJ, len(ortho_pop))  # 使用正交数据量设置参考点数，作为初始种群和
# Decomposition method: defines how a multi-objective problem is decomposed into scalar subproblems, e.g., Tchebycheff, and PBI (Penalty-based Boundary Intersection).
decomp = Tchebicheff()  
# decomp = PBI()
...
csv_file = open(os.path.join(f'{args.outfile}', f'moead_{formula}.csv'), mode='w', newline='')
...
if callback.found_solution:
    ...
    coe = 4
    with open(os.path.join(f'{args.outfile}', f'moead-Target_{formula}_components.txt'), mode="a") as file1, \
         open(os.path.join(f'{args.outfile}', f'moead-Target_{formula}_properties.txt'), mode="a") as file2:
