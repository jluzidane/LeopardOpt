...
class MyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out["F"] = TARGET-calculate_performance(x, NUM_OBJ, formula=formula)
...
ref_dirs = get_reference_directions("energy", n_dim=NUM_OBJ, n_points=len(ortho_pop))
...
csv_file = open(os.path.join(f'{args.outfile}', f'nsga3_{formula}.csv'), mode='w', newline='')
...
if callback.found_solution:
    ...
    coe = 4
    with open(os.path.join(f'{args.outfile}', f'nsga3-Target_{formula}_components.txt'), mode="a") as file1, \
         open(os.path.join(f'{args.outfile}', f'nsga3-Target_{formula}_properties.txt'), mode="a") as file2:
