import numpy as np
import pandas as pd
from platypus import OMOPSO, Problem, Real, TerminationCondition, ParetoDominance, AdaptiveGridArchive
...
algorithm = OMOPSO(
    problem=problem,
    epsilons=[i * 0.05 for i in TARGET],
...
algorithm.archive = AdaptiveGridArchive(
    capacity=100,
    nobjs=NUM_OBJ,
    divisions=3,
...
csv_file = open(os.path.join(f'{args.outfile}', f'omopso_{formula}.csv'), mode='w', newline='')
...
if found_solution:
    ...
    coe = 4
    with open(os.path.join(f'{args.outfile}', f'omopso-Target_{formula}_components.txt'), mode="a") as file1, \
         open(os.path.join(f'{args.outfile}', f'omopso-Target_{formula}_properties.txt'), mode="a") as file2:
